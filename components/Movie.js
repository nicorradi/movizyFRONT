...
function Movie(props) {
  ...
  //Poster URL concatenation
  //console.log(props.poster_path)
  const URLposter = `https://image.tmdb.org/t/p/w500/` + props.poster_path;

  //Limit overview 250 char

const truncatedOverview = props.overview.substr(0, 249) + '[...]';
//console.log(truncatedOverview);
  ...
  return (
    <div className={styles.card}>
      <img className={styles.image} src={URLposter} alt={props.title} />
      <div className={styles.textContainer}>
        <div>
          <span className={styles.name}>{props.title}</span>
          <p className={styles.description}>{truncatedOverview}</p>
        </div>
